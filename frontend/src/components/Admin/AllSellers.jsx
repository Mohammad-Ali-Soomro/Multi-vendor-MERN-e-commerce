...
const AllSellers = () => {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      toast.success("Merchant shop deleted successfully!");
      setOpen(false);
      dispatch(getAllSellers());
    } catch (err) {
  ...
  const columns = [
    ...
    { field: "address", headerName: "Seller Address", minWidth: 180, flex: 1.2 },
    { field: "joinedAt", headerName: "Joined At", minWidth: 130, flex: 0.8 },
    {
      ...
      renderCell: (params) => {
        return (
          <Link to={`/shop/preview/${params.getValue(params.id, "id")}`}>
            <Button>
  ...
  return (
    <div className="w-full p-4 sm:p-8 relative">
      <div className="bg-white rounded-lg shadow-sm border p-6">
        <h5 className="text-[18px] font-[500] text-gray-800 border-b pb-2 mb-4">All Sellers</h5>
        <DataGrid
          rows={rows}
          columns={columns}
          pageSize={10}
        ...
        <div className="fixed w-full h-screen top-0 left-0 bg-[#00000030] z-50 flex items-center justify-center">
          <div className="w-[90%] 800px:w-[35%] bg-white rounded-md p-6 shadow-md text-center relative">
            ...
            <p className="text-gray-500 text-sm mb-6">
              Are you sure you want to delete this merchant shop? This action is permanent and cannot be undone.
            </p>
